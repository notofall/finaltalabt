...
from database import (
    ...
    User, DeliveryRecord, AuditLog, SupplyTracking
)
...
async def confirm_receipt(
    ...
    # ==================== الخصم التلقائي من تتبع التوريد ====================
    # تحديث supply_tracking تلقائياً عند تأكيد استلام العناصر
                ...
                if catalog_item_id:
                    ...
                    supply_result = await session.execute(
                        select(SupplyTracking).where(
                            ...
                            SupplyTracking.catalog_item_id == catalog_item_id
                        )
                    )
                    supply_item = supply_result.scalar_one_or_none()
                    
                    if supply_item:
                        # تحديث الكمية المستلمة في تتبع التوريد
    ...
    await log_audit_pg(
        ...
        f"تأكيد استلام أمر الشراء: {order.order_number} - تم تحديث {supply_updated_count} عنصر في تتبع التوريد"
    )
    ...
    return {
        ...
        "supply_tracking_updated": supply_updated_count
    }
