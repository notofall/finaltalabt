...
const RegisterPage = () => {
  ...
  useEffect(() => {
    // Check if system needs first admin setup - Using V2 API
    axios.get(`${API_BASE}/api/v2/auth/setup/check`)
      .then(res => {
  ...
  const handleCreateFirstAdmin = async (e) => {
    ...
    try {
      // Using V2 API for first admin setup
      const response = await axios.post(`${API_BASE}/api/v2/auth/setup/admin`, adminData);
