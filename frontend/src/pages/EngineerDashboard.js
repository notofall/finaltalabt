...
const EngineerDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Using V2 APIs - Handle paginated response format
      const [requestsRes, statsRes] = await Promise.all([
        axios.get(`${API_V2_URL}/requests/`, getAuthHeaders()),
        axios.get(`${API_V2_URL}/requests/stats`, getAuthHeaders())
      ]);
  ...
  const handleApprove = async (requestId) => {
    ...
    try {
      await axios.post(`${API_V2_URL}/requests/${requestId}/approve`, {}, getAuthHeaders());
  ...
  const handleReject = async () => {
    ...
    setActionLoading(true);
    try {
      await axios.post(`${API_V2_URL}/requests/${selectedRequest.id}/reject`, { reason: rejectionReason }, getAuthHeaders());
  ...
  const handleResubmit = async (requestId) => {
    ...
    setActionLoading(true);
    try {
      await axios.post(`${API_V2_URL}/requests/${requestId}/resubmit`, {}, getAuthHeaders());
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-6 py-4">
        ...
        <div className="mb-4">
          <EngineerReports requests={requests} />
        </div>
