...
const SupervisorDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [requestsRes, statsRes, engineersRes, projectsRes] = await Promise.all([
        axios.get(`${API_V2_URL}/requests/`, getAuthHeaders()),
        axios.get(`${API_V2_URL}/requests/stats`, getAuthHeaders()),
        axios.get(`${API_V2_URL}/auth/users/engineers`, getAuthHeaders()),
  ...
  const searchCatalog = async (searchTerm) => {
    ...
    try {
      ...
      const suggestRes = await axios.get(
        `${API_V2_URL}/catalog/aliases/suggest/${encodeURIComponent(searchTerm)}`,
        getAuthHeaders()
      );
  ...
  const handleRecordDelivery = async () => {
    ...
    setSubmitting(true);
    try {
      await axios.put(`${API_V2_URL}/orders/${selectedDelivery.id}/deliver`, {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-6 py-4">
        ...
        <div className="mb-4">
          <SupervisorReports requests={requests} />
        </div>
