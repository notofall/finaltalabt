...
const QuantityEngineerDashboard = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [statsRes, projectsRes, categoriesRes] = await Promise.all([
        ...
        axios.get(`${API_V2}/quantity/budget-categories`, getAuthHeaders()).catch(() => ({ data: { categories: [] } }))
      ]);
      
      setStats(statsRes.data);
      // Handle paginated V2 response
  ...
  const fetchPlannedItems = useCallback(async () => {
    try {
      ...
      const res = await axios.get(`${API_V2}/quantity/planned?${params.toString()}`, getAuthHeaders());
  ...
  }, [getAuthHeaders, currentPage, searchTerm, filterProject, filterStatus]);
  ...
  const fetchCatalogItems = useCallback(async () => {
    try {
      ...
      const res = await axios.get(`${API_V2}/quantity/catalog-items?${params.toString()}`, getAuthHeaders());
  ...
  }, [getAuthHeaders, catalogPage, catalogSearch]);
