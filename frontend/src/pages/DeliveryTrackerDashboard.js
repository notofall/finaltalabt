...
import { DeliveryReports } from "../components/DashboardReports";

const DeliveryTrackerDashboard = () => {
  const { user, logout, getAuthHeaders, API_V2_URL } = useAuth();
  ...
  const handleSaveInvoiceNumber = async () => {
    ...
    setSubmitting(true);
    try {
      await axios.put(`${API_V2_URL}/orders/${selectedOrder.id}/supplier-invoice`, {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-3 sm:p-4" dir="rtl">
      ...
      <div className="mb-4">
        <DeliveryReports orders={orders} stats={stats} />
      </div>
