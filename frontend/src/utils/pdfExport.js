...
export const fetchAndCacheCompanySettings = async (token) => {
  try {
    ...
    const res = await axios.get(`${API_URL}/api/v2/sysadmin/company-settings/public`, {
...
export const exportRequestToPDF = (request, companySettings = null) => {
  ...
  // Combined header with 3 sections: Company info (right) | Logo (center) | Document info (left)
    ...
    <table style="width: 100%; border: 2px solid ${settings.pdf_primary_color || '#ea580c'}; border-radius: 8px; margin-bottom: 15px; background: linear-gradient(135deg, #fff7ed 0%, #ffffff 100%); border-collapse: separate; border-spacing: 0;">
      <tr>
        ...
        <td style="width: 30%; text-align: center; padding: 15px; vertical-align: middle;">
          ${settings.company_logo ? `<img src="${settings.company_logo}" style="max-height: 70px; max-width: 150px;" />` : ''}
        </td>
        
        <!-- Document Info - Left Side -->
        <td style="width: 35%; text-align: left; padding: 15px; vertical-align: top; border-right: 1px dashed #ddd;">
          <div style="font-size: 22px; font-weight: bold; color: ${settings.pdf_primary_color || '#ea580c'};">طلب مواد</div>
          <div style="font-size: 14px; color: #333; margin-top: 8px;"><strong>رقم:</strong> ${requestNumber}</div>
          <div style="font-size: 12px; color: #666; margin-top: 5px;"><strong>تاريخ:</strong> ${formatDateShort(request.created_at)}</div>
        </td>
      </tr>
    </table>
  `;
