...
const QuantityAlertsReportsManager = () => {
  ...
  const fetchReports = useCallback(async () => {
    try {
      ...
      const res = await axios.get(`${API_V2_URL}/quantity/reports/summary${params}`, getAuthHeaders());
      setReportData(res.data);
    } catch (error) {
      console.error("Error fetching reports:", error);
    }
  }, [API_V2_URL, getAuthHeaders, reportProject]);
  ...
  const handleExportReport = async () => {
    try {
      const params = reportProject ? `?project_id=${reportProject}` : "";
      const response = await axios.get(`${API_V2_URL}/quantity/reports/export${params}`, {
        ...getAuthHeaders(),
        responseType: 'blob'
      });
