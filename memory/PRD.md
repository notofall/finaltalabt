...
- [x] ربط تأكيد الاستلام بنظام كميات العمائر - تحديث supply_tracking تلقائياً عند تأكيد الاستلام (21 يناير 2026)
...
### 3. إضافة شاشة تتبع التوريد للمشرفين والمهندسين ✅
- **الميزة الجديدة**: عرض تتبع التوريد للمشاريع المرتبطة بالمستخدم
- **الملفات المُنشأة**:
  - `frontend/src/components/SupplyTrackingView.js` - مكون عرض تتبع التوريد
  - `backend/routes/v2_buildings_routes.py` - endpoint جديد `/api/v2/buildings/my-supply-tracking`
- **الملفات المُحدّثة**:
  - `frontend/src/pages/SupervisorDashboard.js` - إضافة قسم تتبع التوريد
  - `frontend/src/pages/EngineerDashboard.js` - إضافة قسم تتبع التوريد
- **الميزات**:
  - عرض المشاريع المرتبطة بالمستخدم (أو جميع المشاريع إذا لم يكن هناك ربط)
  - إحصائيات عامة (عدد المشاريع، إجمالي الأصناف، متوسط الإنجاز)
  - توسيع/طي تفاصيل كل مشروع
  - عرض جدول الأصناف مع (المطلوب، المستلم، المتبقي، نسبة الإنجاز)
  - شريط بحث لفلترة المشاريع
  - رسالة توضيحية عن التحديث التلقائي للكميات

### 4. إعادة تصميم نافذة إنشاء أمر الشراء ✅ (22 يناير 2026)
- **المشكلة**: النافذة القديمة كانت طويلة ومعقدة وغير متجاوبة على الموبايل
- **الحل الجديد**:
  - إنشاء مكون منفصل `OrderCreationDialog.js` بتصميم Mobile-First
  - استخدام **Accordion Sections** لتنظيم الأقسام (الأصناف، المورد، الميزانية، الملاحظات)
  - إضافة **Progress Bar** يظهر 4 خطوات: الأصناف، المورد، الربط، جاهز
  - أزرار **Sticky Footer** ثابتة في الأسفل (إصدار أمر شراء + طلب عرض سعر)
  - عرض تنبيه للأصناف غير المربوطة بالكتالوج
  - حساب الإجمالي تلقائياً
  - تصميم متجاوب 100% على الموبايل
- **الملفات المُحدّثة**:
  - `frontend/src/components/OrderCreationDialog.js` - المكون الجديد
  - `frontend/src/pages/ProcurementDashboard.js` - دمج المكون الجديد
- **نتائج الاختبار**: 100% نجاح لجميع الميزات
