...
### 1. إزالة المشاريع من نظام الكميات ✅
- **المشكلة**: لا يمكن حذف/إزالة المشاريع من نظام الكميات فقط
- **الحل**: 
  - إضافة حقل `is_building_project` في جدول المشاريع
  - إضافة API endpoints جديدة:
    - `GET /api/v2/buildings/projects` - عرض المشاريع المُفعّلة في نظام الكميات فقط
    - `DELETE /api/v2/buildings/projects/{id}` - إزالة مشروع من نظام الكميات (إخفاء وليس حذف)
    - `POST /api/v2/buildings/projects/{id}/enable` - إعادة تفعيل مشروع في نظام الكميات
  - إضافة زر الإزالة (🗑️) بجانب كل مشروع في واجهة نظام الكميات
- **الملفات**: 
  - `backend/database/models.py` - إضافة حقل is_building_project
  - `backend/routes/v2_buildings_routes.py` - إضافة APIs الجديدة
  - `frontend/src/pages/BuildingsSystem.js` - تحديث الواجهة
